...
fowardSimMetaContinue<- function(simPop,lociSim,ploidy,DRR,popSizeSims,numPopsSims,n1,
 ...
 # matrixT1<-as.data.frame(matrixT1)
  ...
  matrixT1<- simPop[[1]]
  matrixT2<-matrixT1
  matrixT2[, 4:(3 + ploidy*lociSim)]  <- 0
  locusBounds <- simPop[[2]]
  allelicStates <- simPop[[3]]
  # Generation Tally
  GenerationsSoFar<- simPop[[8]]
  ...
  for (thisGen in 1:(n1+n2)) {
      ...
      resultsForwardSim[["Generations"]] <- n1 + n2 + GenerationsSoFar
