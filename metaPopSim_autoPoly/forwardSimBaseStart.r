...
fowardSimBase<- function(lociSim,ploidy,DRR,popSizeBase,n1,mutationRate,mutationModel) {
  ...
  matrixT1<-matrix(0,popSizeBase,3+length(lociLabels))
  colnames(matrixT1)<-c("ID","pop","mother",lociLabels)
  rownames(matrixT1)<-1:popSizeBase
  matrixT1[,"ID"]<-1:popSizeBase
  popLabels<-"ancestral"
  matrixT1[,"pop"]<-popLabels
  matrixT1[,"mother"]<-NA
  #matrixT1<-as.data.frame(matrixT1)
  # Empty matrix for later
  matrixT2<-matrixT1
  #setkey(matrixT1,ID)

  ######################################################
  # Starting mimimum variability (1 allele per locus)  #
  ######################################################
  ...
  for (thisLocus in 1:lociSim) {
      ...
      resultsForwardSim[["Generations"]]<-n1
